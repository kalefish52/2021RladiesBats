...
# to get get to here you can type getwd() in console and that will give you where you at.. 
...
names(data) # can also look at readme file (you can find this in the website https://datadryad.org/stash/dataset/doi:10.5061/dryad.3td78sk , it explain which each name in the  table means)
names(data)
# whene you type function name and <- it will save the info under ENvironment so later you don't have to look up the entire thing
...
batspec<-unique(data["Forearm"]) 

print(paste("There are", nrow(batspec), "Forearm in the dataset."))


# Reduce dataframe
data[c("Habitat", "Forearm")]
unique(data[c("Habitat", "Species")]) 
...
SpeciesRich<-aggregate(Species~Habitat,
                     data=SpeciesRed,
                     FUN=length)
#you can code different name and classes of the families and you create a graph... google some tech for coding the graphs and how to create them use the link to see how to create new graphs http://www.cookbook-r.com/Graphs/


# ggsave is to save the graph or figure in any folder in your computer. 
# Change here

# Graph it 
data$Forearm<-as.numeric(data$Forearm)
data$Forearm
hist(data$Forearm)
ggplot(data=SpeciesRich, aes(x=Habitat, y=Species)) +
    geom_bar(colour="black", stat="identity")
ggsave("./Figures/histForearm.png")
